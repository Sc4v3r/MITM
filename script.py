...
• Here’s the complete nac-monitor.py with the fixes and manual “Delete PCAP” workflow applied:
...
"""
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  <script>
  let startTime=null,currentFilter='all',allLoot=[];
  function showAlert(message,type="info"){const el=document.getElementById("alert");el.className=`alert alert-${type}
  show`;el.textContent=message;setTimeout(()=>el.classList.remove("show"),5000)}
  function formatBytes(bytes){if(bytes===0)return"0 B";const k=1024,sizes=["B","KB","MB","GB"];const i=Math.floor(Math.log(bytes)/Math.log(k));return parseFloat((bytes/
  Math.pow(k,i)).toFixed(2))+" "+sizes[i]}
  function formatDuration(seconds){const h=Math.floor(seconds/3600);const m=Math.floor(seconds%3600/60);const
  s=Math.floor(seconds%60);return`${h.toString().padStart(2,"0")}:${m.toString().padStart(2,"0")}:${s.toString().padStart(2,"0")}`}
  function switchTab(tab){document.querySelectorAll(".tab-content").forEach(el=>el.classList.remove("active"));document.querySelectorAll(".tab-
  btn").forEach(el=>el.classList.remove("active"));document.getElementById(tab+"Tab").classList.add("active");event.target.classList.add("active");if(tab==="loot")fetchLo
  ot()}
  function updateStatus(data){const isActive=data.status==="active";const badge=document.getElementById("statusBadge");badge.className=`status-badge
  ${isActive?"active":"inactive"}`;badge.innerHTML=`<span class="status-indicator ${isActive?"active":"inactive"}"></span><span>${isActive?"Capturing":"Inactive"}
  </span>`;const captureInfo=document.getElementById("captureInfo");captureInfo.className=`capture-info ${isActive?"active":""}
  `;document.getElementById("clientIP").textContent=data.client_ip||"-";document.getElementById("gatewayIP").textContent=data.gateway_ip||"-";if(isActive)
  {document.getElementById("captureFile").textContent=data.pcap_file?
  data.pcap_file.split("/").pop():"-";document.getElementById("capturePid").textContent=data.pid||"-";document.getElementById("bridgeName").textContent=data.bridge||"br0"
  ;const size=data.pcap_size||0;const packets=data.packet_count||
  0;document.getElementById("captureSize").textContent=formatBytes(size);document.getElementById("capturePackets").textContent=packets.toLocaleString()+"
  packets";if(data.start_time){if(!startTime)try{startTime=new Date(data.start_time)}catch(e){startTime=new Date}const elapsed=Math.floor((new
  Date()-startTime)/1000);document.getElementById("captureDuration").textContent=formatDuration(elapsed);document.getElementById("captureStartTime").textContent="Started
  "+startTime.toLocaleTimeString()}
  document.getElementById("btnStart").disabled=true;document.getElementById("btnStop").disabled=false;document.getElementById("btnDelete").disabled=true;document.getEleme
  ntById("btnDownload").disabled=false}
  else{document.getElementById("btnStart").disabled=false;document.getElementById("btnStop").disabled=true;document.getElementById("btnDelete").disabled=!
  data.pcap_file;document.getElementById("btnDownload").disabled=!data.pcap_file;startTime=null;document.getElementById("captureSize").textContent=data.pcap_size?
  formatBytes(data.pcap_size):"0 MB";document.getElementById("capturePackets").textContent="0
  packets";document.getElementById("captureDuration").textContent="00:00:00";document.getElementById("captureStartTime").textContent="Not started"}if(data.interfaces)
  {const container=document.getElementById("interfaces");container.innerHTML=data.interfaces.map(intf=>`<div class="interface-card"><div class="interface-
  header"><span class="interface-name">${intf.name}</span><span class="interface-status ${intf.state==="UP"?"up":"down"}">${intf.state}</span></div><div
  class="interface-detail"><span>MAC:</span><span style="font-family:monospace">${intf.mac||"N/A"}</span></div><div class="interface-detail"><span>Speed:</
  span><span>${intf.speed||"Unknown"}</span></div><div class="interface-detail"><span>Role:</span><span>${intf.role||"N/A"}</span></div>${intf.bridge?
  `<div class="interface-detail"><span>Bridge:</span><span>${intf.bridge}</span></div>`:""}</div>`).join("")}if(data.logs&&data.logs.length>0){const
  logEl=document.getElementById("logs");logEl.innerHTML=data.logs.map(line=>`<div style="padding:2px 0">${line}</div>`).join("");logEl.scrollTop=logEl.scrollHeight}}
  async function analyzeNow(){document.getElementById("btnAnalyze").disabled=true;showAlert("Running PCredz analysis... This may take a few minutes.","info");try{const
  res=await fetch("/api/analyze",{method:"POST"});const data=await res.json();if(data.success){if(data.new_items>0){showAlert(`Analysis complete! Found ${data.new_items}
  credential(s)`,"success");fetchLoot()}else{showAlert("Analysis complete. No new credentials found.","info")}}else{showAlert("Analysis failed: "+(data.error||"Unknown
  error"),"error")}}catch(err){showAlert("Error: "+err.message,"error")}finally{document.getElementById("btnAnalyze").disabled=false}}
  async function deletePCAP(){if(confirm("Delete current PCAP file? This cannot be undone!")){try{const res=await fetch("/api/delete_pcap",{method:"POST"});const
  data=await res.json();if(data.success){showAlert("PCAP deleted","success");fetchStatus();fetchLoot()}else{showAlert("Failed to delete PCAP: "+(data.error||"Unknown
  error"),"error")}}catch(err){showAlert("Error: "+err.message,"error")}}}
  document.getElementById("btnDelete").addEventListener("click",deletePCAP);
  async function fetchStatus(){try{const res=await fetch("/api/status");const data=await res.json();updateStatus(data)}catch(err){console.error("Failed:",err)}}
  async function fetchLoot(){try{const res=await fetch("/api/loot");const data=await res.json();allLoot=data.items||
  [];document.getElementById("lootBadge").textContent=data.count||0;document.getElementById("totalLoot").textContent=data.count||
  0;document.getElementById("protocolCount").textContent=Object.keys(data.protocols||{}).length;displayLoot(allLoot)}catch(err){console.error("Failed to fetch
  loot:",err)}}
  function displayLoot(items){const container=document.getElementById("lootItems");if(!items||items.length===0){container.innerHTML='<div class="empty-state"><div
  class="empty-state-icon">🎣</div><p>No credentials captured yet</p><p style="margin-top:10px;font-size:.9em">Start capture and wait for traffic analysis</
  p></div>';return}container.innerHTML=items.map(item=>`<div class="loot-item ${item.protocol.toLowerCase()}"><div class="loot-header"><span class="loot-
  protocol">${item.protocol}</span><span class="loot-timestamp">${new Date(item.timestamp).toLocaleString()}</span></div><div class="loot-content"><div class="loot-
  field">Username: <strong>${item.username}</strong></div><div class="loot-field">Password: <strong>${item.password}</strong></div><div class="loot-field" style="margin-
  top:10px;font-size:.85em;color:#666">Source: ${item.source}</div></div></div>`).join("")}
  function filterLoot(filter){currentFilter=filter;document.querySelectorAll(".filter-btn").forEach(btn=>{const
  label=btn.textContent.trim();btn.classList.toggle("active",(filter==="all"&&label==="All")||label===filter)});const filtered=filter==="all"?
  allLoot:allLoot.filter(item=>item.protocol===filter);displayLoot(filtered)}
  async function exportLoot(){window.location.href="/api/loot/export"}
  async function clearLoot(){if(confirm("Clear all captured credentials?")){try{const res=await fetch("/api/loot/clear",{method:"POST"});const data=await
  res.json();if(data.success){showAlert("Loot cleared","success");fetchLoot()}else{showAlert("Failed to clear loot","error")}}catch(err){showAlert("Error:
  "+err.message,"error")}}}
  async function startCapture(){document.getElementById("btnStart").disabled=true;showAlert("Starting packet capture...","info");try{const res=await fetch("/api/
  start",{method:"POST"});const data=await res.json();if(data.success){showAlert("Capture started!","success");setTimeout(fetchStatus,2000);setTimeout(fetchLoot,3000)}
  else{showAlert("Failed to start. Check logs.","error");document.getElementById("btnStart").disabled=false}}catch(err){showAlert("Error:
  "+err.message,"error");document.getElementById("btnStart").disabled=false}}
  async function stopCapture(){if(confirm("Stop capture? Bridge will remain active.")){document.getElementById("btnStop").disabled=true;showAlert("Stopping
  capture...","info");try{const res=await fetch("/api/stop",{method:"POST"});const data=await res.json();if(data.success){showAlert("Capture stopped!","success")}
  else{showAlert("Failed to stop","error")}setTimeout(fetchStatus,3000);setTimeout(fetchLoot,4000)}catch(err){showAlert("Error: "+err.message,"error")}}}
  async function downloadPCAP(){try{const res=await fetch("/api/status");const data=await res.json();if(data.pcap_file){window.location.href="/api/download?
  file="+encodeURIComponent(data.pcap_file)}else{showAlert("No file","error")}}catch(err){showAlert("Error: "+err.message,"error")}}
  document.getElementById("btnStart").addEventListener("click",startCapture);
  document.getElementById("btnStop").addEventListener("click",stopCapture);
  document.getElementById("btnRefresh").addEventListener("click",()=>{fetchStatus();fetchLoot()});
  document.getElementById("btnDownload").addEventListener("click",downloadPCAP);
